...
2. **Search the web for a real matching post, then verify it's actually the same face.** Default behavior, not opt-in: every scan runs a genuine reverse-image search (Google Lens via SerpAPI, with Serper.dev as a fallback), then downloads each candidate's thumbnail and re-checks it against the query face with our own ArcFace/SFace embeddings. This is what lets it find a *different* photo of the same person, not just a repost of the exact same image - a Vox article, an Instagram post, and a news photo all get recognized as the same person even though Lens found them as three unrelated images. Results are cached by image hash so re-scanning the same photo never spends API quota twice.
...
- **Search results are re-verified by face, not trusted on photo similarity alone.** Google Lens matches by visual similarity, which finds the exact same photo reposted elsewhere reliably, but on its own won't distinguish a genuinely different photo of the same person from a photo that just looks similar. Every candidate Lens returns gets its thumbnail downloaded and run back through our own ArcFace/SFace embedding, cosine-compared against the query face, and re-ranked by that real similarity score - so the result that wins is the one that's actually verified as the same face, not just the one Lens ranked first. In testing this reliably surfaces different photos of the same person (different Instagram posts, different articles) with 0.75+ similarity scores.

...
- **Every search result is re-verified by face, not trusted on photo similarity alone.** Google Lens matches by visual similarity, which finds the exact same photo reposted elsewhere reliably, but on its own won't distinguish a genuinely different photo of the same person from a photo that just looks similar. Every candidate Lens returns (up to 20 per search) gets its thumbnail downloaded and run back through our own ArcFace/SFace embedding, cosine-compared against the query face, and kept only if it's actually the same person. In testing on a well-known face this surfaces 15-20+ different photos from completely different sources (news sites, Instagram, different articles), each independently confirmed as the same person rather than just visually similar.
